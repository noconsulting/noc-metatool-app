      ...
      <section className='mb-8'>
        ...
        <div className='p-4 bg-gray-50 rounded-lg'>
          ...
          <div className='relative mb-6'>
            <button
              onClick={() => {
                const endpoint = `https://noc-metatool-proxy.srv817320.hstgr.cloud/sse with Authorization: Bearer ${apiKey?.api_key ?? '<create an api key first>'}`;
            ...
            <Highlight
              theme={themes.github}
              code={`https://noc-metatool-proxy.srv817320.hstgr.cloud/sse with Authorization: Bearer ${apiKey?.api_key ?? '<create an api key first>'}`}
              language='bash'>
          ...
          <div className='relative'>
            <button
              onClick={() => {
                const endpoint = `https://noc-metatool-proxy.srv817320.hstgr.cloud/api-key/${apiKey?.api_key ?? '<create an api key first>'}/sse`;
                navigator.clipboard.writeText(endpoint);
                toast({
            ...
            <Highlight
              theme={themes.github}
              code={`https://noc-metatool-proxy.srv817320.hstgr.cloud/api-key/${apiKey?.api_key ?? '<create an api key first>'}/sse`}
              language='bash'>
              {({ tokens, getLineProps, getTokenProps }) => (
                <pre className='bg-[#f6f8fa] text-[#24292f] p-4 rounded-md overflow-x-auto'>
                  {tokens.map((line, i) => (
